# Same as Emcee_Dynamic_Dark_Energy.py except the parameters are more simplified to imitate Lambda CDM model
...
def main():
    ...
    def log_posterior(params):
        stacked_ps = np.load("stacked_ps_5_param_w0_wa_-0.9_-0.2_1_3.npy", allow_pickle=True)
        cov = np.load("cov_5_param_w0_wa_-0.9_-0.2_1_3.npy", allow_pickle=True)
        min_ell = 75
        max_ell = 1000
        n = 10

        def log_prior(params):
            omega_c, n_s, lnas = params
            if 0.1 < omega_c < 0.9 and 0.8 < n_s < 1.2 and 1.61 < lnas < 3.91:
                return 0.0
            return -np.inf
        ...
        def log_likelihood(params):
            ...
            model = CosmologicalModel(h=h, omega_b=omega_b, omega_c=omega_c, n_s=n_s, a_s=a_s, max_ell=max_ell)
        ...
        omega_c, n_s, lnas = params
        a_s = np.exp(lnas) / (10**10)
        cosmo_model = CosmologicalModel(h=h, omega_b=omega_b, omega_c=omega_c, n_s=n_s, a_s=a_s, max_ell=max_ell)
        return lp + ll, lp, cosmo_model.model.sigma8()

    ndim = 3  # number of parameters
    nwalkers = 16  # adjusted number of walkers
    nsteps = 2000  # adjusted number of steps
    burn_in_iteration = 2 # burn-in iteration
    each_burn_in_step = 500 # each burn in step

    pos = np.random.uniform(low=[0.1, 0.8, 1.61], high=[0.9, 1.2, 3.91], size=(nwalkers, ndim))
    ...
    base_name = "chain_results_3_16_1000_2000_w0_wa_-0.9_-0.2_1_3"
    blob_name = "blob_results_3_16_1000_2000_w0_wa_-0.9_-0.2_1_3"
    base_name_prob = "prob_results_3_16_1000_2000_w0_wa_-0.9_-0.2_1_3"
